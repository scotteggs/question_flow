 ...
 * @return Promise
 */
export function makeQuestionnaireRequest(method, id, data, api = '/questionnaire') {
  return request[method](api + (id ? ('/' + id) : ''), data);
}


// can remvoe increment, decrement
...
export function createTopic(text) {
  return (dispatch, getState) => {
    ...
    return makeQuestionnaireRequest('post', id, data)
      .then(res => {
        if (res.status === 200) {
...
export function fetchQuestionnaires() {
  return {
    type: types.GET_QUESTIONNAIRES,
    promise: makeQuestionnaireRequest('get')
  };
}
